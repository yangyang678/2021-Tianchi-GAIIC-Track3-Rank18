...
from model.modeling_nezha import NeZhaModel, BertOnlyMLMHead
...
class MyNeZha(nn.Module):
    def __init__(self, config, keep_tokens=None, resume=False):
        super(MyNeZha, self).__init__()
        self.config = config
        if resume:
            self.nezha = NeZhaModel(config=self.config)
        else:
            self.nezha = NeZhaModel.from_pretrained(
                './nezha-cn-wwm/pytorch_model.bin',
                config=self.config)
        ...
        if keep_tokens is not None:
            ...
            weight_nezha = torch.load('embeddingNeZhaWWM.pth')
            weight_nezha['weight'] = weight_nezha['weight'][keep_tokens]
            weight = nn.Parameter(weight_nezha['weight'])
            embedding.weight = weight
            self.nezha.embeddings.word_embeddings = embedding
...
def train(args, model):
    ...
    pretrain_dataset = NeZhaDataset(train_data, tokens, args.max_seq_length)
    ...
    for epoch in range(args.resume_epoch, args.epoch):
        ...
        if args.local_rank == 0:
            if (epoch + 1) == 90:
                torch.save(model.module.state_dict(), args.model_save_dir + 'wwm-distribute-epoch{}.pth'.format(epoch + 1),
...
def main():
    ...
    parser.add_argument(
        "--bert_config_path",
        default='./nezha-cn-wwm/bert_config.json',
        type=str,
    ...
    config = NeZhaConfig.from_json_file(args.bert_config_path)
    keep_tokens = []
    ...
    if args.do_resume:
        ...
        model = MyNeZha(config=config, resume=True)
        ...
        model = MyNeZha(config=config, keep_tokens=keep_tokens)
        #print(model)
        model.cuda(args.local_rank)
        #model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.local_rank], output_device=args.local_rank, find_unused_parameters=True)
        train(args, model)
