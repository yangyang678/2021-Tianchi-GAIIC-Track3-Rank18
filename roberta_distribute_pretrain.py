...
class MyRoberta(nn.Module):
    def __init__(self, config, keep_tokens=None, resume=False):
        ...
        if resume:
            ...
            self.roberta = BertModel.from_pretrained('./roberta-cn-wwm/pytorch_model.bin', config=self.config)
        ...
        if keep_tokens is not None:
            ...
            weight_roberta = torch.load('embeddingRoberta.pth')
...
def train(args, model):
    ...
    for epoch in range(args.resume_epoch, args.epoch):
        ...
        if args.local_rank == 0:
            if (epoch + 1) == 150:
                torch.save(model.module.state_dict(), args.model_save_dir + f'roberta-distribute-epoch{epoch + 1}.pth',
...
def main():
    ...
    parser.add_argument(
        "--bert_config_path",
        default='./roberta-cn-wwm/bert_config.json',
        type=str,
