...
class MyNeZha(nn.Module):
    def __init__(self, config, keep_tokens=None, resume=False):
        ...
        if keep_tokens is not None:
            ...
            weight_nezha = torch.load('embeddingNeZhaBase.pth')
...
def train(args, model):
    ...
    for epoch in range(args.resume_epoch, args.epoch):
        ...
        if args.local_rank == 0:
            if (epoch + 1) == 90:
                torch.save(model.module.state_dict(), args.model_save_dir + 'base-distribute-epoch{}.pth'.format(epoch + 1),
...
def main():
    ...
    parser.add_argument(
        "--bert_config_path",
        default='./nezha-cn-base/config.json',
        type=str,
