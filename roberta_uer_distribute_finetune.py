...
class Model(nn.Module):
    def __init__(self, args, config):
        ...
        self.args = args
        self.roberta = BertModel(config=config)
        #self.fc_input_dims = 737  # cls
        self.fc_input_dims = 705
    ...
    def forward(self, x):
        output = self.roberta(**x)[0]  # 0:sequence_output  1:pooler_output
...
def train(args, model):
    ...
    train_dataset = RobertaDataset(train_data, vocab_dict, args.maxlen)
    ...
    for e in range(args.epoch):
        ...
        torch.save(model.state_dict(), args.model_save_path + f'/uer_{args.struc}_best_model.pth',
...
def predict(args, model):
    ...
    test_data = pd.read_csv('./tcdata/gaiic_track3_round1_testB_20210317.tsv', sep='\t',
                            names=['text_a', 'text_b', 'label'])
    test_data['label'] = -100

    test_dataset = RobertaDataset(test_data, vocab_dict, args.maxlen, predict=True)
...
def main():
    ...
    parser.add_argument("--bert_config", default='./roberta-cn-uer/config.json', type=str)
    ...
    NeZhaconfig = RobertaConfig.from_json_file(args.bert_config)
    ...
    if args.do_train:
        ...
        for name in file_list:
            ...
            if os.path.isfile(model_path) and name.split('-')[0] == 'uer':
